...
static uint32_t last_bulk_event_type = 0;
static uint16_t last_bulk_event_sent = 0;
static uint16_t last_bulk_event_total = 0;
static uint32_t last_suspend_ms = 0;
static uint32_t last_resume_ms = 0;
...
//--------------------------------------------------------------------
// EP81 interrupt notifications
//
// This Pico SDK/TinyUSB build does not expose tud_edpt_open/busy/xfer.
// EP81 is already present in the USB descriptor, but without a public
// endpoint API we must not try to drive it directly from application code.
// Keep these functions as safe no-ops so the firmware builds; the critical
// Broadcom WLC_E_IF event is sent through TinyUSB vendor bulk IN below.
//--------------------------------------------------------------------
static void open_ep1_in(void) {
    if (!ep1_in_opened) {
        ep1_in_opened = true;
        printf("EP81 descriptor present; direct app xfer disabled for this TinyUSB build\r\n");
    }
}
...
static void send_interrupt_event(void) {
    ...
    if (ep81_first_event) {
        ...
        printf("EP81 first heartbeat skipped: no public endpoint API in this pico-sdk\r\n");
    }

    notification_counter++;
    if ((notification_counter & 0x0F) == 1) {
        printf("EP81 heartbeat skipped #%lu: using EP82 bulk event path\r\n",
               (unsigned long)notification_counter);
    }
}

//--------------------------------------------------------------------
// Send full Broadcom event packet on runtime bulk IN (vendor IN endpoint)
// Includes eth header (14 bytes) + bcmeth header (10 bytes) + event_msg
//--------------------------------------------------------------------
static void send_bulk_event(uint32_t event_type, uint32_t status, uint32_t reason, uint32_t flags) {
    // USB BCDC data frames carry a 4-byte BCDC header before the Ethernet frame.
    // Layout:
    //   0..3   BCDC header: proto v2, priority 0, flags2 0, data_offset 0
    //   4..17  Ethernet header
    //   18..27 Broadcom bcmeth header
    //   28..75 wl_event_msg_t / brcmf_event_msg_be (48 bytes)
    //   76..80 wl_event_data_if_t payload (5 bytes)
    // Total = 81 bytes. Bulk MPS=64 -> host receives 64 + 17.
    uint8_t pkt[84];
    memset(pkt, 0, sizeof(pkt));

    const uint16_t bcdc_off = 0;
    const uint16_t eth_off = 4;
    const uint16_t bcm_off = eth_off + 14;
    const uint16_t msg_off = bcm_off + 10;
    const uint16_t payload_off = msg_off + 48;
    const uint16_t total_pkt = payload_off + 5;

    // === BCDC header (4 bytes) ===
    // flags: BCDC protocol version 2 << 4 = 0x20
    pkt[bcdc_off + 0] = 0x20;
    pkt[bcdc_off + 1] = 0x00; // priority
    pkt[bcdc_off + 2] = 0x00; // flags2 / ifidx bits
    pkt[bcdc_off + 3] = 0x00; // data_offset

    // === Ethernet header (14 bytes) ===
    // dst = broadcast
    pkt[eth_off + 0] = 0xFF; pkt[eth_off + 1] = 0xFF; pkt[eth_off + 2] = 0xFF;
    pkt[eth_off + 3] = 0xFF; pkt[eth_off + 4] = 0xFF; pkt[eth_off + 5] = 0xFF;
    // src = EMU_MAC (6 bytes)
    memcpy(pkt + eth_off + 6, EMU_MAC, 6);
    // ethertype = 0x886C (Broadcom / ETH_P_LINK_CTL)
    pkt[eth_off + 12] = 0x88; pkt[eth_off + 13] = 0x6C;

    // === bcmeth_hdr_t (10 bytes) ===
    // subtype = BCMILCP_SUBTYPE_VENDOR_LONG = 0x8001 (big-endian)
    pkt[bcm_off + 0] = 0x80; pkt[bcm_off + 1] = 0x01;
    // length = 59 = version(1) + oui(3) + usr_subtype(2) + event_msg(48) + payload(5)
    pkt[bcm_off + 2] = 0x00; pkt[bcm_off + 3] = 0x3B;
    // version = 0, oui = 00:10:18, usr_subtype = 1 (event)
    pkt[bcm_off + 4] = 0x00;
    pkt[bcm_off + 5] = 0x00; pkt[bcm_off + 6] = 0x10; pkt[bcm_off + 7] = 0x18;
    pkt[bcm_off + 8] = 0x00; pkt[bcm_off + 9] = 0x01;

    // === wl_event_msg_t / brcmf_event_msg_be (48 bytes, big-endian) ===
    pkt[msg_off + 0] = 0x00; pkt[msg_off + 1] = 0x02; // version = 2
    pkt[msg_off + 2] = (uint8_t)((flags >> 8) & 0xFF);
    pkt[msg_off + 3] = (uint8_t)(flags & 0xFF);
    pkt[msg_off + 4] = (uint8_t)((event_type >> 24) & 0xFF);
    pkt[msg_off + 5] = (uint8_t)((event_type >> 16) & 0xFF);
    pkt[msg_off + 6] = (uint8_t)((event_type >>  8) & 0xFF);
    pkt[msg_off + 7] = (uint8_t)(event_type & 0xFF);
    pkt[msg_off + 8] = (uint8_t)((status >> 24) & 0xFF);
    pkt[msg_off + 9] = (uint8_t)((status >> 16) & 0xFF);
    pkt[msg_off + 10] = (uint8_t)((status >>  8) & 0xFF);
    pkt[msg_off + 11] = (uint8_t)(status & 0xFF);
    pkt[msg_off + 12] = (uint8_t)((reason >> 24) & 0xFF);
    pkt[msg_off + 13] = (uint8_t)((reason >> 16) & 0xFF);
    pkt[msg_off + 14] = (uint8_t)((reason >>  8) & 0xFF);
    pkt[msg_off + 15] = (uint8_t)(reason & 0xFF);
    // auth_type = 0
    pkt[msg_off + 16] = 0x00; pkt[msg_off + 17] = 0x00; pkt[msg_off + 18] = 0x00; pkt[msg_off + 19] = 0x00;
    // datalen = 5 (sizeof wl_event_data_if_t)
    pkt[msg_off + 20] = 0x00; pkt[msg_off + 21] = 0x00; pkt[msg_off + 22] = 0x00; pkt[msg_off + 23] = 0x05;
    memcpy(pkt + msg_off + 24, EMU_MAC, 6);       // addr
    memcpy(pkt + msg_off + 30, "wl0", 3);         // ifname[16], zero-padded
    // ifidx=0 and bsscfgidx=0 are already zero at msg offsets 46..47

    // === wl_event_data_if_t payload (5 bytes) ===
    pkt[payload_off + 0] = 0x00;  // ifidx
    pkt[payload_off + 1] = 0x01;  // opcode/action = BRCMF_E_IF_ADD
    pkt[payload_off + 2] = 0x00;  // reserved/flags
    pkt[payload_off + 3] = 0x00;  // bssidx
    pkt[payload_off + 4] = 0x00;  // role = STA

    uint16_t sent = 0;
    uint8_t retries = 0;

    while (sent < total_pkt && retries < 8) {
        uint32_t n = tud_vendor_write(pkt + sent, total_pkt - sent);
        if (n == 0) {
            tud_vendor_flush();
            retries++;
            continue;
    ...
    last_bulk_event_type = event_type;
    last_bulk_event_sent = sent;
    last_bulk_event_total = total_pkt;

    printf("EP82/vendor BCDC event %lu sent: %u/%u bytes%s hdr=%02X %02X %02X %02X eth=%02X%02X type=%02X%02X len=%02X%02X\r\n",
           (unsigned long)event_type,
           sent,
           total_pkt,
           sent == total_pkt ? "" : " PARTIAL",
           pkt[0], pkt[1], pkt[2], pkt[3],
           pkt[4], pkt[5], pkt[eth_off + 12], pkt[eth_off + 13], pkt[bcm_off + 2], pkt[bcm_off + 3]);
}
...
//--------------------------------------------------------------------
// Runtime USB state log helpers
//--------------------------------------------------------------------
static void log_usb_runtime_state(const char *tag) {
    uint32_t now = to_ms_since_boot(get_absolute_time());
    printf("USBDBG %s t=%lu boot=%u postboot=%u dongle_up=%u fw=%lu dl_state=%lu pending_if=%lu runtime_pending=%u out_len=%u last_cmd=%lu last_len=%lu flags=0x%08lX last_iovar='%s' last_evt=%lu %u/%u last_suspend=%lu last_resume=%lu\r\n",
           tag,
           (unsigned long)now,
           (unsigned)boot_mode,
           (unsigned)postboot,
           ...
           (unsigned long)pending_if_event_ms,
           ...
           (unsigned long)last_bulk_event_type,
           last_bulk_event_sent,
           last_bulk_event_total,
           (unsigned long)last_suspend_ms,
           (unsigned long)last_resume_ms);
}
...
void tud_mount_cb(void) {
    ...
    log_usb_runtime_state("MOUNT enter");
    if (boot_mode) {
        status_led_boot();
    } else {
        status_led_runtime();
    ...
    log_usb_runtime_state("MOUNT exit");
}
void tud_umount_cb(void) {
    log_usb_runtime_state("UMOUNT before-clear");
    printf("USB unmounted\r\n");
    ep1_in_opened = false;
    log_usb_runtime_state("UMOUNT after-clear");
}
void tud_suspend_cb(bool remote_wakeup_en) {
    last_suspend_ms = to_ms_since_boot(get_absolute_time());
    printf("USB suspend ENTER t=%lu remote_wakeup_en=%d\r\n",
           (unsigned long)last_suspend_ms, remote_wakeup_en);
    log_usb_runtime_state("SUSPEND before-led-off");
    status_led_off();
    log_usb_runtime_state("SUSPEND after-led-off");
}
void tud_resume_cb(void) {
    last_resume_ms = to_ms_since_boot(get_absolute_time());
    printf("USB resume ENTER t=%lu\r\n", (unsigned long)last_resume_ms);
    log_usb_runtime_state("RESUME before-actions");
    if (!boot_mode) {
        status_led_runtime();
        send_interrupt_event();
        tud_remote_wakeup();
    ...
    log_usb_runtime_state("RESUME after-actions");
}
...
int main(void) {
    ...
    printf("\r\n=== CYW4373 Emulator v4.26 BCDC-header event + suspend trace ===\r\n");
