...
static void send_bulk_event(uint32_t event_type, uint32_t status, uint32_t reason, uint32_t flags) {
    ...
    memcpy(pkt + 48, EMU_MAC, 6);       // addr
    memcpy(pkt + 54, "wl0", 3);         // ifname[16], zero-padded
    // ifidx=0 and bsscfgidx=0 are already zero at offsets 70..71

    // === wl_event_data_if_t payload (5 bytes), offset 72 ===
    pkt[72] = 0x00;  // ifidx
    ...
    while (sent < total_pkt && retries < 6) {
    ...
    printf("EP82/vendor event %lu sent: %u/%u bytes%s\r\n",
