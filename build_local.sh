...
static void open_ep1_in(void) {
    ...
    ep1_desc.xfer             = TUSB_XFER_INTERRUPT;
    ep1_desc.sync             = 0;
    ep1_desc.usage            = 0;
    ep1_desc.wMaxPacketSize   = 16;
    ep1_desc.bInterval        = 9;

    bool ok = tud_edpt_open(&ep1_desc);
    ep1_in_opened = true; // Descriptor may already have opened EP81; treat this as non-fatal.
    printf("EP81 open: %s\r\n", ok ? "OK" : "already active/open failed nonfatal");
}
...
static void send_bulk_event(uint32_t event_type, uint32_t status, uint32_t reason, uint32_t flags) {
    ...
    if (tud_edpt_busy(0x82)) {
        printf("EP82 event %lu xfer busy: 0/%u bytes\r\n", event_type, total_pkt);
        return;
    }

    bool ok = tud_edpt_xfer(0x82, pkt, total_pkt);
    printf("EP82 event %lu xfer %s: %u bytes\r\n",
           event_type, ok ? "OK" : "FAIL", ok ? total_pkt : 0);
}
